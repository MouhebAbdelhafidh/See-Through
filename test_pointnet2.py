#!/usr/bin/env python3
"""
Test script for PointNet2 feature extraction
"""
...
if not Path(POINTNET2_PATH).exists():
    print(f"PointNet2_PyTorch not found. Please run setup_pointnet2.py first.")
    sys.exit(1)
...
try:
    ...
    print("✓ PointNet2_PyTorch imported successfully")
except ImportError as e:
    print(f"✗ Failed to import PointNet2: {e}")
    print("Please run setup_pointnet2.py first")
    sys.exit(1)

from pointnet2_feature_extractor import PointNet2Backbone, preprocess_point_cloud
...
def test_model():
    ...
    if output.shape == (batch_size, 128, num_points):
...
def test_feature_extraction():
    ...
    from pointnet2_feature_extractor import extract_features_from_file
    ...
    try:
        ...
        if features.shape[1] == 64:  # feature_dim
...
def main():
    """Run all tests"""
    print("Running PointNet2 feature extraction tests...\n")
    ...
    if passed == total:
        print("✓ All tests passed! PointNet2 feature extraction is ready to use.")
        return True
    else:
